...
-spec split(List, N, _Lists, Start) -> [list()] when List::list(),
						    N::integer(),
						    Start::integer().
...
split(L, N, Lists, Start) ->
    {L1, L2} = lists:split(N, L),
    if length(L2) > N ->
	    split(L2, N, [{Start,L1}|Lists],Start+N);
       true ->
	    [{Start,L1}, {Start+N,L2}|Lists]
    end.
...
collect(N, Maxes) when length(Maxes) < N ->
	...
	{'EXIT', _PID, random_death} ->
	    %%	{'EXIT', _PID, random_death, L, Death} ->
	    %%	collect(N, [worker(L, self(), Death) |Maxes]);
	    %%  TODO: restart worker, but how?
	    collect(N, [-1 |Maxes]); 
	{'EXIT', _PID, normal} ->
	    collect(N, Maxes);
	Max -> 
