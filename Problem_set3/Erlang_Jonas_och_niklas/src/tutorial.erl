...
right_triangles(N) ->
    L = lists:seq(1, N),
    Triangles = [{A,B,C} || A <- L, B <- L, C <- L, A*A + B*B =:= C*C]. 
...
simpsons(names) ->
    Names = [X || {_,_,X}<- simpsons()] ;
simpsons(males) ->
    Males = [Y || {_,X,Y} <- simpsons(), X == male];
simpsons(females) ->
    Females = [Y || {_,X,Y} <- simpsons(), X == female];
simpsons(pets) ->
    Pets = [Y || {X,_,Y} <- simpsons(), X =/= person].
...
char_to_upper(Char) when Char > 96 andalso Char < 123 ->
    Char-32.
...
char_to_lower(Char) when Char > 64 andalso Char < 91 ->
    Char+32.
