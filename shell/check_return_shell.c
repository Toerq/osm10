...
void child_command(enum cmd_pos pos, char* argv[], int left_pipe_read_fd, int right_pipe[]) {
  ...
  if(signal(SIGPIPE, sigpipe_handler) == SIG_ERR) { perror("Signal fail...\n"); exit(EXIT_FAILURE); }
  ...
  case first:
    //close(left_pipe_read_fd);
    if(dup2(right_pipe[1], STDOUT_FILENO) == -1) { perror("Dup2 fail...\n"); exit(EXIT_FAILURE); }
    if(close(right_pipe[1]) == -1) { perror("Close fail...1\n"); exit(EXIT_FAILURE); }
    if(close(right_pipe[0]) == -1) { perror("Close fail...2\n"); exit(EXIT_FAILURE); }
    break;
  case middle:
    if(dup2(left_pipe_read_fd, STDIN_FILENO) == -1) { perror("Dup2 fail...\n"); exit(EXIT_FAILURE); }
    if(close(left_pipe_read_fd) == -1) { perror("Close fail...3\n"); exit(EXIT_FAILURE); }
    if(dup2(right_pipe[1], STDOUT_FILENO) == -1) { perror("Dup2 fail...\n"); exit(EXIT_FAILURE); }
    if(close(right_pipe[1]) == -1) { perror("Close fail...4\n"); exit(EXIT_FAILURE); }
    if(close(right_pipe[0]) == -1) { perror("Close fail...5\n"); exit(EXIT_FAILURE); }
    break;
  case last:
    if(dup2(left_pipe_read_fd, STDIN_FILENO) == -1) { perror("Dup2 fail...\n"); exit(EXIT_FAILURE); }
    if(close(left_pipe_read_fd) == -1) { perror("Close fail...6\n"); exit(EXIT_FAILURE); }
    //close(right_pipe[0]);
    //close(right_pipe[1]);
    break;
  default:
...
int pipe_and_fork(enum cmd_pos pos, char* argv[], int left_pipe_read_fd) {
  ...
  switch(pos){
  case first:
    if(pipe(new_pipe) == -1) { perror("Pipe fail...\n"); exit(EXIT_FAILURE); }
    piped = 1;
    break;
  case middle:
    if(pipe(new_pipe) == -1) { perror("Pipe fail...\n"); exit(EXIT_FAILURE); }
  ...
  default: // == PARENT ==
    ...
    //close(left_pipe_read_fd);	  
    if(piped && (close(new_pipe[1]) == -1)) { perror("Close fail...\n"); exit(EXIT_FAILURE); }	  
